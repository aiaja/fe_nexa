...
export function FilterPopover({ onApply, currentFilters }: FilterPopoverProps) {
  ...
  const [selectedRoles, setSelectedRoles] = useState<UserRole[]>(currentFilters.roles)
  const [selectedStatuses, setSelectedStatuses] = useState<UserStatus[]>(currentFilters.statuses)
  const [isReady, setIsReady] = useState(false)

  const userRoles: UserRole[] = ["Super Admin", "Manager", "Auditor", "Supervisor"]
  const userStatuses: UserStatus[] = ["Active", "Under Review", "Suspended"]
        ...
        <div className="p-4">
          ...
          <FieldGroup className="space-y-4">
            <div className="space-y-2">
              <h4 className="text-xs font-medium text-gray-700">User Role</h4>
              <div className="grid grid-cols-2 gap-2">
                {userRoles.map((role) => (
                  <Field key={role} orientation="horizontal" className="items-center space-x-2">
                    <Checkbox
                      id={`role-${role}`}
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-medium text-gray-700">User Status</h4>
